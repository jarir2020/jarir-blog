<template>
    <div>
        ...
        <p v-if="!loading && !error && categories.length === 0" class="text-sm text-gray-500 dark:text-gray-400">
            <template v-if="hasActiveFilters">No categories match your search.</template>
            <template v-else>No categories yet.</template>
        </p>

        <div v-if="!loading && categories.length > 0" class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
            <table class="min-w-full">
                ...
                <tbody>
                    <tr v-for="c in categories" :key="c.id" class="border-t border-gray-200 dark:border-gray-700">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ c.posts_count ?? 0 }}</td>
