<template>
    <div>
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">Statuses</h2>
            <router-link
        ...
        <p v-if="!loading && !error && statuses.length === 0" class="text-sm text-gray-500">
            <template v-if="hasActiveFilters">No statuses match your search.</template>
            <template v-else>No statuses yet.</template>
        </p>

        <div v-if="!loading && statuses.length > 0" class="bg-white shadow-sm rounded-lg overflow-hidden">
            <table class="min-w-full">
                ...
                <tbody>
                    <tr v-for="s in statuses" :key="s.id" class="border-t border-gray-200">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">{{ s.order }}</td>
                        <td class="px-4 py-3 text-sm text-gray-500">{{ s.description || '—' }}</td>
