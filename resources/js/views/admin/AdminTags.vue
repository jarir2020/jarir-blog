<template>
    <div>
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Tags</h2>
            <router-link
        ...
        <p v-if="!loading && !error && tags.length === 0" class="text-sm text-gray-500 dark:text-gray-400">
            <template v-if="hasActiveFilters">No tags match your search.</template>
            <template v-else>No tags yet.</template>
        </p>

        <div v-if="!loading && tags.length > 0" class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
            <table class="min-w-full">
                ...
                <tbody>
                    <tr v-for="t in tags" :key="t.id" class="border-t border-gray-200 dark:border-gray-700">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ t.posts_count ?? 0 }}</td>
